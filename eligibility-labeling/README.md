...
inclusion / exclusion criterion strings are not labeled. On save, the trial-
level `cancerTypes` field is derived as the union of cohort applicable types;
both the trial-level and per-cohort cancer-type arrays are sorted in registry
order so the JSON is stable across labelers.
