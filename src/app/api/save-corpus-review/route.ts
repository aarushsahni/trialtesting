// HTTP endpoint mirroring saveCorpusReviewAction — needed because
// navigator.sendBeacon (used on tab close / browser back) can't call
// Next.js server actions directly. Same auth + update logic.
...
export async function POST(req: NextRequest) {
  ...
  if (!rows[0]) return NextResponse.json({ error: 'no editable review' }, { status: 409 });
